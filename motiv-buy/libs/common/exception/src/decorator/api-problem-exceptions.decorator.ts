...
export function ApiProblemExceptions(
  ...
  decorators.push(
    ...[...statusExceptionMap.entries()].map(([status, items]) => {
      // NOTE: We will add more useful description later
      // const oneOfDescriptions = items.map(
      //   ([exception, options]) =>
      //     `  * ${options.description ?? exception.title ?? formatTitleFromClassName(exception.name)}`,
      // );
      //
      // const description = `${getHttpStatusName(status)}, variants (oneOf):<br />${oneOfDescriptions.join('<br />')}`;

      const examples: { [key: string]: ApiResponseExamples } = {};

      const schemas = items.map(([exception, options]) => {
        // Get problemType and title from static properties
