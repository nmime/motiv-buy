    ...
    for (const phase of phases) {
      ...
      if (validLatencies.length > 0) {
        phaseAnalysis[phase] = {
          ...
          contributionPercent:
            (validLatencies.reduce((sum, l) => sum + l, 0) / measurements.reduce((sum, m) => sum + m.totalLatency, 0)) *
            100,
        };
      }
    }
...
class AdaptiveOptimizer {
    ...
    for (const [scenario, result] of results) {
      if (result.throughput && result.throughput.optimalThroughput < result.throughput.maxThroughput * 0.8) {
        bottlenecks.push({
          ...
          severity: 'HIGH',
          impact:
            (result.throughput.maxThroughput - result.throughput.optimalThroughput) / result.throughput.maxThroughput,
  ...
  async applyOptimizations(optimizations) {
    ...
    const sortedOptimizations = optimizations.sort(
      (a, b) => b.confidence * parseFloat(b.expectedImprovement) - a.confidence * parseFloat(a.expectedImprovement),
    );
