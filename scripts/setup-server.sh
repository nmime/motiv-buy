...
if [ "$PROTOCOL" = "http" ]; then
    ...
    echo ""
    echo "  3. Test deployer SSH access (root keys auto-copied):"
    echo -e "     ${CYAN}ssh $DEPLOY_USER@$SERVER_IP${NC}"
    echo ""
    echo "  4. (Optional) Add deployment key for GitHub Actions:"
    echo -e "     ${CYAN}ssh-keygen -t ed25519 -C \"deploy-$ENV_TYPE\" -f ~/.ssh/motiv-$ENV_TYPE${NC}"
    echo -e "     ${CYAN}ssh-copy-id -i ~/.ssh/motiv-$ENV_TYPE.pub $DEPLOY_USER@$SERVER_IP${NC}"
    echo ""
    echo "  5. Configure GitHub Secrets (see docs/DEPLOY.md)"
    echo ""
    echo "  6. Deploy from GitHub Actions"
else
    echo "  1. Test deployer SSH access (root keys auto-copied):"
    echo -e "     ${CYAN}ssh $DEPLOY_USER@$SERVER_IP${NC}"
    echo ""
    echo "  2. (Optional) Add deployment key for GitHub Actions:"
    echo -e "     ${CYAN}ssh-keygen -t ed25519 -C \"deploy-$ENV_TYPE\" -f ~/.ssh/motiv-$ENV_TYPE${NC}"
    echo -e "     ${CYAN}ssh-copy-id -i ~/.ssh/motiv-$ENV_TYPE.pub $DEPLOY_USER@$SERVER_IP${NC}"
    echo ""
    echo "  3. Configure GitHub Secrets (see docs/DEPLOY.md)"
    echo ""
    echo "  4. Deploy from GitHub Actions"
fi
echo ""
